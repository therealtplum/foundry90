...
4. **Save location**: 
   - Navigate to `capstones/therealtplum/clients/` (one level up from `F90Mobile/`)
   - **Important**: If Xcode warns "A folder named 'F90Mobile' already exists", click **Cancel**
   - Instead, navigate to `clients/` directory and save the project there
   - The project file will be created at: `clients/F90Mobile.xcodeproj`
   - This way your existing `F90Mobile/` directory with source files remains intact

5. **Add existing source files**:
   - In Xcode's project navigator, you'll see an auto-generated `F90Mobile` folder/group
   - Delete the auto-generated `F90MobileApp.swift` and `ContentView.swift` files
   - Right-click on the `F90Mobile` group in Xcode's project navigator
   - Select "Add Files to F90Mobile..."
   - Navigate to `clients/F90Mobile/F90Mobile/` (where your existing source files are)
   - Select:
     - `F90MobileApp.swift`
     - `MainTabView.swift`
